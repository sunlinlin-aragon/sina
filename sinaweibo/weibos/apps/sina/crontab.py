...
def send_sina_weibo(*args, **kwargs):
    try:
        ...
        send_articles = Article.objects.filter(send_datetime__gte = datetime.datetime.now(),  send_datetime__lt=datetime.datetime.now() + datetime.timedelta(minutes=10), is_send = False)
